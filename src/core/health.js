...
async function checkForUpdate() {
  if (_updateCache && Date.now() - _updateCache.at < 3600_000)
    return _updateCache.value;
  let value = null;
  try {
    ...
    if (localSha && m) {
      ...
      const remoteSha = await new Promise((resolve) => {
        const req = http.get(
          {
            host: 'api.github.com',
            path: `/repos/${repo}/commits/HEAD`,
            headers: {
              'User-Agent': 'tradingview-mcp',
          ...
          (res) => {
            ...
            res.on('data', (c) => (d += c));
            res.on('end', () =>
              resolve(res.statusCode === 200 ? d.trim() : null),
            );
          },
        );
      ...
      if (remoteSha) {
        value = {
          ...
          ...(remoteSha !== localSha && {
            hint: 'Run the tv_update tool (or `tv update` CLI) to update, then restart the MCP server.',
          }),
        };
      }
    }
...
async function _probeCdp(cdpPort) {
  ...
  return new Promise((resolve) => {
    const req = http.get(
      `http://${CDP_HOST}:${cdpPort}/json/version`,
      (res) => {
        let data = '';
...
function _spawnFailedEarly(child, graceMs = 1500) {
  return new Promise((resolve) => {
    ...
    const onError = (e) => {
      cleanup();
      resolve(e.code || e.message || 'spawn error');
    };
    const onExit = (code) => {
      cleanup();
      resolve(`exited immediately (code ${code})`);
    };
    const cleanup = () => {
      clearTimeout(timer);
 ...
 */
function _copyMsixPackageLocal(
  tvPath,
  { cpSync, rmSync, readdirSync, existsSync },
) {
