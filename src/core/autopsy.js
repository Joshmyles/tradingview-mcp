 ...
 * timeout fires. `restored` in the result says what went back and whether it
 * was verified, and a restore that FAILED is reported as an error on an
 * otherwise successful packet rather than being swallowed — leaving the live
 * research chart on a forensics layout is the worst outcome here, and silence
 * about it is worse still.
 *
...
import { resolutionSeconds } from './chart.js';
...
async function captureContext(ev) {
  return ev(`
    (function() {
      ...
      var out = { symbol: null, resolution: null, layout: null, layout_id: null, range: null };
...
async function switchLayout(ev, id) {
  await ev(`window.TradingViewApi.loadChartFromServer({ id: ${JSON.stringify(id)} })`);
  // The load is not instantaneous and exposes no edge of its own; the settle
  // barrier after it is what actually proves the chart is usable again.
  for (let i = 0; i < 60; i++) {
    await sleep(500);
...
export async function lossAutopsy({
  ...
  try {
    if (layoutTarget && layoutTarget.name !== before.layout) {
      ...
      const now = await switchLayout(ev, layoutTarget.id);
    ...
    try {
      ...
      if (restored.attempted && before.layout && now.layout !== before.layout) {
        ...
        if (back) {
          const got = await switchLayout(ev, back.id);
      ...
      restored.matches_as_found =
        final.symbol === before.symbol &&
        final.resolution === before.resolution &&
        final.layout === before.layout;
      if (!restored.matches_as_found) {
        restored.error =
          'The chart was NOT returned to the state it was found in. ' +
          `Found ${before.layout} / ${before.symbol} / ${before.resolution}; left on ${final.layout} / ${final.symbol} / ${final.resolution}. Put it back by hand.`;
