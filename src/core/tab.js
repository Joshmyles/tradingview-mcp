...
export async function list() {
  ...
  const tabs = targets
    .filter(
      (t) =>
        t.type === 'page' &&
        (/tradingview\.com\/chart/i.test(t.url) || t.title === 'New tab'),
    )
    .map((t, i) => ({
      index: i,
      id: t.id,
...
async function withShell(fn) {
  ...
  const candidates = targets.filter(
    (t) => t.type === 'page' && /\/window\/index\.html/i.test(t.url || ''),
  );
  ...
  throw new Error(
    'TradingView shell window (tab bar) not found. Is this TradingView Desktop with tabs?',
  );
}
...
async function findLandingTarget() {
  ...
  const targets = await resp.json();
  return (
    targets.find((t) => t.type === 'page' && t.title === 'New tab') || null
  );
}
...
export async function newTab({ layout, name } = {}) {
  ...
  if (!landing) {
    shellCounts = await withShell(async (evalIn) => {
      ...
      `);
      if (!clicked)
        throw new Error('New-tab button not found in shell window.');
  ...
  if (!landing)
    throw new Error(
      'New tab opened but its landing page target was not found.',
    );
  ...
  const picked = await withTarget(landing.id, async (evalIn) => {
    if (wantNew) {
      ...
      await evalIn(
        `(function(){ var b = document.querySelector('.create-new-layout-button'); if (b) b.click(); })()`,
      );
      ...
      if (filled !== 'filled')
        throw new Error(
          `Create-layout dialog did not open as expected (${filled}).`,
        );
      ...
      if (!created)
        throw new Error(
          'Create button not found or still disabled in the layout dialog.',
        );
      return layoutName;
    }
    const clickByTitle = `
    ...
    if (!foundTitle) {
      ...
      await evalIn(
        `(function(){ var b = document.querySelector('.layout-list-expand-button'); if (b) b.click(); })()`,
      );
  ...
  if (!picked)
    throw new Error(
      `Layout matching "${layout}" not found in the layout list.`,
    );
  ...
  for (let i = 0; i < 30; i++) {
    ...
    chartTarget =
      targets.find(
        (x) =>
          x.type === 'page' &&
          /tradingview\.com\/chart/i.test(x.url) &&
          !chartIdsBefore.has(x.id),
      ) ||
      targets.find(
        (x) => x.id === landing.id && /tradingview\.com\/chart/i.test(x.url),
      ) ||
      null;
    if (chartTarget) break;
  }
  if (!chartTarget)
    throw new Error(`Picked "${picked}" but no new chart target appeared.`);
...
export async function closeTab() {
  const before = await withShell((evalIn) =>
    evalIn(`document.querySelectorAll('.tabs-container .tab').length`),
  );
  if (before <= 1) {
    throw new Error(
      'Cannot close the last tab. Use tv_launch to restart TradingView instead.',
    );
  }
...
export async function switchTab({ index }) {
  ...
  if (idx >= tabs.tab_count) {
    throw new Error(
      `Tab index ${idx} out of range (have ${tabs.tab_count} tabs)`,
    );
  }
  ...
  if (!(await isTargetVisible(target.id))) {
    ...
    if (clicked === null) {
      throw new Error(
        `Clicked through all shell tabs but chart ${target.chart_id} never became visible.`,
      );
    }
  }
  ...
  try {
    ...
    throw new Error(
      `Tab is visible but failed to re-attach CDP to it: ${e.message}`,
    );
  }
